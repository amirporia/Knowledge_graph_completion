...
def filter_known_answers(scores: torch.Tensor, head_ids: List[str], relations: List[str],
                         target_idx: torch.Tensor, all_triplet_dict: TripletDict,
                         entity_dict: EntityDict, broad_filter: bool = False,
                         tail_ids: Optional[List[str]] = None) -> None:
    """In-place filtered-ranking mask: for each row i, sets the score of
    every entity that is a *known* correct answer for (head_ids[i],
    relations[i]) -- other than the target itself -- to -inf, following the
    standard filtered evaluation protocol (Sec 7.2).

    broad_filter (default False -- the standard protocol, and what ARPM-KGC's
    own reported numbers use): when True, ALSO masks any entity that is a
    predecessor of the head under ANY relation, not just the query's own
    relation. This reproduces Baseline's `_collect_mask_indices` behavior
    for parity comparisons. It is not part of the standard filtered-ranking
    protocol -- it only ever removes additional candidates, making ranking
    strictly easier -- so leave it off for ARPM-KGC's own numbers. Requires
    `tail_ids` (the gold tail per row) to replicate Baseline's exclusion of
    the query's own answer from this extra mask.
    """
    if broad_filter and tail_ids is None:
        raise ValueError("broad_filter=True requires tail_ids to be passed.")

    for i in range(scores.size(0)):
        known = all_triplet_dict.get_neighbor_tails(head_ids[i], relations[i])
        if not broad_filter and len(known) <= 1:
            continue
        known = set(known)
        if broad_filter:
            gold_tail = tail_ids[i]
            known |= {h for h in all_triplet_dict.get_predecessors(head_ids[i]) if h != gold_tail}
...
@torch.no_grad()
def apply_neighbor_rerank_bonus(scores: torch.Tensor, head_ids: List[str], link_graph: LinkGraph,
                                entity_dict: EntityDict, weight: float, n_hop: int) -> None:
    """In-place: adds `weight` to the score of every training-graph entity
    within `n_hop` hops of each row's head entity. Reproduces
    Baseline/utils/utils.py's `rerank_by_graph` heuristic (Baseline default:
    weight=0.02, n_hop=2), which Baseline's own evaluate.py applies
    unconditionally. This is a hand-tuned structural bonus, not part of the
    model's learned score -- OFF by default here (weight=0.0), so ARPM-KGC's
    own numbers are unaffected. Turn it on only to reproduce a
    Baseline-equivalent number.
    """
    if weight == 0.0:
        return
    for i in range(scores.size(0)):
        neighbor_ids = link_graph.get_n_hop_neighbor_ids(head_ids[i], n_hop=n_hop)
        if not neighbor_ids:
            continue
        idx = torch.tensor([entity_dict.entity_to_idx(e) for e in neighbor_ids], device=scores.device)
        bonus = torch.full((idx.numel(),), weight, device=scores.device, dtype=scores.dtype)
        scores[i].index_add_(0, idx, bonus)
