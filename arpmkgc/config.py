...
class ARPMConfig:
    ...
    # ------------------------------------------------------------------
    # Optional evaluation-time extras, OFF by default -- i.e. ARPM-KGC's
    # own reported numbers are completely unaffected unless you opt in.
    # These exist only to let `arpmkgc.evaluate` reproduce two heuristics
    # that `Baseline/evaluation/evaluate.py` applies unconditionally, so
    # the two codebases can be compared on equal footing. Turn them ON
    # only when you specifically want a Baseline-equivalent number.
    # ------------------------------------------------------------------
    eval_neighbor_rerank_weight: float = 0.02
    eval_neighbor_rerank_n_hop: int = 2
    eval_broad_answer_filter: bool = True
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    fair = parser.add_argument_group("Evaluation extras (Baseline-parity, opt-in)")
    fair.add_argument("--eval-neighbor-rerank-weight", type=float,
                       default=defaults.eval_neighbor_rerank_weight,
                       help="Add this weight to scores of every n-hop training-graph neighbor of "
                            "the head before ranking (0 = off; Baseline's default is 0.02). "
                            "Reproduces Baseline's rerank_by_graph heuristic.")
    fair.add_argument("--eval-neighbor-rerank-n-hop", type=int,
                       default=defaults.eval_neighbor_rerank_n_hop)
    _add_bool_arg(fair, "eval-broad-answer-filter", defaults.eval_broad_answer_filter,
                  "Also mask candidates that are a predecessor of the head under ANY relation "
                  "(Baseline's _collect_mask_indices behavior). Off = standard filtered-ranking "
                  "protocol used by ARPM-KGC's own numbers.")
    ...
    if ablation_name is not None:
        ...
        cli_overrides = {
            ...
            if k in ("train_path", "valid_path", "test_path", "model_dir",
                     ...
                     "eval_neighbor_rerank_weight", "eval_neighbor_rerank_n_hop",
                     "eval_broad_answer_filter")
               and v is not None
        }
