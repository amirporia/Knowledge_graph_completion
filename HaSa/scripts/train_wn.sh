...
# `NPROC_PER_NODE=1 ./scripts/train_wn.sh` to force single-GPU training even on
# a multi-GPU machine.
#
...
# training from the last saved epoch, e.g.:
#   ./scripts/train_wn.sh --resume --model-dir "$OUTPUT_DIR"
set -x
set -e

TASK="wn18rr"

DIR="$( cd "$( dirname "$0" )" && cd .. && pwd )"             # this pipeline's own root, e.g. .../HaSa
...
if [ -z "$DATA_DIR" ]; then
  # Shared with SimKGC, StAR and ARPM_KGC -- one preprocessed copy of the data
  # for every pipeline, e.g. F:\KGC\Knowledge_graph_completion\data\wn18rr
  # (previously this pointed at "${DIR}/data/${TASK}", a HaSa-local copy that
  # doesn't exist and used mixed-case "WN18RR" instead of "wn18rr").
