...
class Example:
    ...
    def vectorize(self) -> dict:
        ...
        # HaSa encodes head / relation / tail as three *separate* texts (see
        # models.py's module docstring) rather than concatenating head+relation like
        # SimKGC/StAR, so a standalone relation-only tokenization is needed too.
...
def collate(batch_data: List[dict]) -> dict:
    ...
    batch_dict = {
        ...
        # No triplet_mask / self_negative_mask here: unlike SimKGC/StAR, HaSa's loss
        # (see trainer.py::_hasa_loss) doesn't use in-batch masking, so skip the
        # (non-trivial, O(batch^2)) mask construction entirely rather than compute it
        # and throw it away every step.
    }
