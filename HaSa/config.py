...
# file lives at <repo_root>/HaSa/config.py, so its parent's parent is
...
# -- there is no separate HaSa/data copy.
REPO_ROOT = Path(__file__).resolve().parent.parent
PIPELINE_DIR = Path(__file__).resolve().parent

parser = argparse.ArgumentParser(description='HaSa arguments')
...
parser.add_argument('--embedding-dim', default=500, type=int, metavar='N',
                    help='Output dimension of HaSa\'s f(.) encoder (Linear+LayerNorm+Dropout '
                         'projection, see models.py::HaSaBertModel.proj) and of the GRU '
                         'aggregator g(.,.). BUGFIX: scripts/train_wn.sh and '
                         'scripts/train_fb.sh already pass --embedding-dim 500, but this '
                         'flag was never declared here, so argparse rejected it with '
                         '"unrecognized arguments: --embedding-dim 500" -- now declared '
                         'with the same default the scripts use.')
...
# point at this same shared folder instead of a HaSa-local data/ copy).
# ------------------------------------------------------------------------------
